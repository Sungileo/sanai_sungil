...
```{r}
data_age_raw <- read.csv("~/dataset/행정안전부_지역별(법정동) 성별 연령별 주민등록 인구수_20230430.csv",fileEncoding = "CP949")
...
```{r}
load("~/dataset/forensic_glass.rda")
forensic_glass %>% select(-type,-RI,-Si) %>%
  ggcorr(palette = "RdBu",label =TRUE)
```

```{r}
load("~/dataset/CO2_emissions.rda")
...
```{r}
load("~/dataset/preprint_growth.rda")
...
```{r}
load("~/dataset/dow_jones_industrial.rda")
