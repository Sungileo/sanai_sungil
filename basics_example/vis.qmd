...
description: "Data Visualization with ggplot"
image: "vis.png"
categories: "R"
editor_options: 
...
```{r}
data_file <- read.csv("C:/sanai_sungil/basics_example/ncdc_normals.csv")
```
...
## Visualizing proportions: 비율 데이터의 시각화

```{r}
bundestag <- read.csv("https://raw.githubusercontent.com/Sungileo/trainsets_2/main/bundestag.csv")
bundestag$party
```
...
### Figure 10.1
bund_pie %>%  ggplot(aes(x="", y=seats, fill=party_fac)) +
  geom_bar (stat="identity") +
  geom_text(aes(x = 1, y = ypos, label = seats), color = "white", size=5) +
  geom_text(aes(x = 1.5, y = ypos, label = party, hjust = hjust, vjust = vjust),
            color="black", size=5) +
...
```{r}
marketshare <- read.csv("https://raw.githubusercontent.com/Sungileo/trainsets_2/main/marketshare.csv")

marketshare %>% ggplot(aes(x=company,y=percent,fill=company))+
  geom_col()+
  facet_wrap(~year)+
...
bridges <- read.csv('https://raw.githubusercontent.com/Sungileo/trainsets_2/main/bridges.csv')
...
bridges_sum <- select(bridges,MATERIAL,ERECTED) %>% 
  table() %>% 
  reshape2::melt(vlaue.name = "count")
...
bridges_sum %>% ggplot(aes(area = value, 
                           subgroup = MATERIAL,
                           fill = interaction(ERECTED,MATERIAL)))+
  geom_treemap(color = "white", size = 0.5*.pt, alpha = NA)+
  geom_treemap_subgroup_text(colour = "grey50",
                             place = "center",
                             alpha = 0.7,
                             grow = T)+
  geom_treemap_subgroup_border(color = "white")+
  geom_treemap_text(aes(label = ERECTED,
                        color = interaction(ERECTED,MATERIAL)),
                    color = "black",
                    place = "center",
                    grow = F)+
...
population_df_color %>% ggplot(aes(area=area,subgroup = region, fill=fill))+
  geom_treemap(color="white", size = 0.5*.pt, alpha=NA)+
...
blue_jays <- read.csv("https://raw.githubusercontent.com/nishamathi/SampleDatasets/main/blue_jays.csv")
blue_jays$sex <- factor(blue_jays$sex,levels = c("F","M"))

blue_jays %>% ggplot(aes(body_mass_g,head_length_mm))+
...
```{r}
blue_jays %>% ggplot(aes(body_mass_g,head_length_mm,fill=sex))+
...
```{r}
blue_jays$sex_2 <- ifelse(blue_jays$sex == "F","female birds","male birds")
blue_jays$sex_2 <- factor(blue_jays$sex_2, levels = c("female birds","male birds"))

blue_jays %>% ggplot(aes(body_mass_g,head_length_mm,size = skull_size_mm,fill=sex))+
  geom_point(pch=21,color="white")+
  facet_wrap(~sex)+
  xlab("Body mass (g)")+
  ylab("Head length (mm)")+
  scale_fill_manual(values = c(`F`="#D55E00",`M`="#0072b2"),guid = "none")+
  scale_radius(
    range = c(2,7),
    limits = c(28,34),
...
```{r}
load("C:/trainsets_2/forensic_glass.rda")
forensic_glass %>% select(-type,-RI,-Si) %>%
  ggcorr(palette = "RdBu",label =TRUE)
```

```{r}
load("C:/trainsets_2/CO2_emissions.rda")

emissions_1970_2010 <- CO2_emissions %>% filter(year %in% c(1970,2010)) %>% 
  tidyr::spread(year,emissions) %>% na.omit()

emissions_1970_2010 %>% ggplot(aes(`1970`,`2010`))+
...
```{r}
load("C:/trainsets_2/preprint_growth.rda")

biorxiv_growth <- preprint_growth %>% 
  filter(archive == "bioRxiv") %>% 
  filter(count > 0)

biorxiv_growth %>% ggplot(aes(date,count))+
...
```{r}
biorxiv_growth %>% ggplot(aes(date,height = count, y=0))+
...
```{r}
preprint_final <- filter(preprint,date == "2017-01-01")
...
preprint %>% 
  ...
  scale_y_continuous(limits = c(0,600),
                     ...
                     sec.axis = dup_axis(breaks = preprint_final$count,
                                         labels = c("arXiv\nq-bio","PeerJ\nPreprints","bioRxiv"),
                                         name = NULL))+
  scale_x_date(name = "Year",
...
```{r}
load("C:/trainsets_2/dow_jones_industrial.rda")
...
dow_jones_industrial %>% 
  ...
  geom_line(linewidth = 0.75)+
  geom_smooth(aes(color = "smooth"),size = 1,na.rm = T,se = F,
              method = "gam",formula = y ~ s(x, k = 3, bs = "cr"))+
  scale_color_manual(name = NULL,
                     values = c(close = "#000000",smooth = "#0072b2"),
...
```{r}
blue_jays %>% 
  ggplot(aes(body_mass_g,head_length_mm,fill=sex,color=sex))+
  geom_point(pch=21,color = "white",size=3)+
...
CO2_detrended <- 
  mutate(CO2_complete,
         ...
         `seasonal fluctration` = CO2_stl$time.series[,"seasonal"],
         `long-term trend` = CO2_stl$time.series[,"trend"],
         remainder = CO2_stl$time.series[,"remainder"])

facet_labels <- c("monthly average","seasonal fluctration","long-term trend","remainder")
...
ggp <- CO2_detrended %>% 
  ...
  geom_line(linewidth=0.75,color = "#0072b2")+
  facet_wrap(facets = ~variable, ncol=1,scale="free")+
  theme_minimal()
...
# Function Definition
write.pptx <- function(ggp, out.pptx.file, title="", append = F, 
                       left = 0.5, top = 1.7, width = 9, height = 5){
  # read pptx
  if(append){
    out <- officer::read_pptx(out.pptx.file)
  } else {
  ...
  out <- out %>% 
    officer::add_slide() %>%
    officer::ph_with(rvg::dml(ggobj = ggp), 
                     officer::ph_location(left = left, 
                                          top = top, 
                                          width = width, 
                                          height = height))
  # add title
  if(title!=""){
    out <- out %>% 
      officer::ph_with(value = title, 
                       location = officer::ph_location_type(type = "title"))
  }
  base::print(out, target = out.pptx.file)
}

```

```{r}
#ggsave("co2.png",co2,path = "C:/Sungil_LAB",width = 16,height = 9)

#htmlwidgets::saveWidget(img,"imgname.html")

#plotly::ggplotly(ggp) %>% htmlwidgets::saveWidget("ggp.html")

#write.pptx(ggp,"figure 14,3.pptx")
#write.pptx(ggp,"figure 14,3.pptx", title = "fiigure 14.3",append = T)
